...
const { FiGrid, FiList, FiPlus, FiCode } = FiIcons;
      ...
      const mockTasks = [
        {
          ...
          description: 'Redesign the client portal with new React components and improved UX',
          task_type: 'react',
          status: 'In Progress',
          priority: 'High',
        ...
        },
        {
          ...
          description: 'Create React components for real-time analytics dashboard',
          task_type: 'react',
          status: 'To Do',
          priority: 'Medium',
  ...
  const handleSaveTask = async (taskData) => {
    try {
      if (editingTask) {
        ...
        const { error } = await supabase
          .from('tasks')
          .insert([{ ...taskData, task_type: 'react' }]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center">
              <SafeIcon icon={FiCode} className="w-5 h-5 text-purple-600" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-gray-900">React Tasks</h1>
              <p className="text-gray-600">Development and React-specific tasks</p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: 'Total React Tasks', value: tasks.length, color: 'bg-purple-100 text-purple-800' },
      ...
      {tasks.length === 0 && (
        <div className="text-center py-12">
          <SafeIcon icon={FiCode} className="w-12 h-12 text-gray-400 mx-auto mb-4" />
          <h3 className="text-lg font-medium text-gray-900 mb-2">No React tasks found</h3>
          <p className="text-gray-600 mb-4">
            Get started by creating your first React development task.
          </p>
          <button
            onClick={() => setShowTaskForm(true)}
